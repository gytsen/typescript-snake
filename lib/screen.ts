...
export class Screen {
    ...
    constructor(canvas: HTMLCanvasElement, borderSize: number = 3, boxSize: number = 20) {
        if (canvas.width % boxSize !== 0 || canvas.height % boxSize !== 0) {
            throw new Error(`${canvas.height} or ${canvas.width} is not divisible by ${boxSize}`);
        }
    ...
    public drawCoordinates(coordinates: Iterable<Coordinate>, color: string = BLACK): void {
    ...
    public drawCoordinate(coordinate: Coordinate, color: string = BLACK): void {
        ...
        this._context.fillRect(canvasPoint.x, canvasPoint.y, this._trimmedBoxSize, this._trimmedBoxSize);
